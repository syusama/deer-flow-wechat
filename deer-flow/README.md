...
| WeChat | wechat-link (long-polling) | Moderate |
...
channels:
  ...
  wechat:
    enabled: true
    # Optional: override the built-in implementation with a third-party class
    # class_path: your_package.wechat:WeChatChannel
    bot_token: $WECHAT_BOT_TOKEN    # optional when session_file already stores bot_token
    base_url: https://ilinkai.weixin.qq.com
    # Generated by `cd backend && uv run deerflow-wechat-login --session-file ../.state/wechat-link-session.json`
    session_file: ./.state/wechat-link-session.json
    cursor_file: ./.state/wechat-cursor.json   # keep this persistent to avoid replaying old updates
```

Notes:
- `assistant_id: lead_agent` calls the default LangGraph assistant directly.
- If `assistant_id` is set to a custom agent name, DeerFlow still routes through `lead_agent` and injects that value as `agent_name`, so the custom agent's SOUL/config takes effect for IM channels.
- Any channel can override its implementation with `channels.<name>.class_path`, which lets third-party channel packages plug into `ChannelService` without editing DeerFlow core.
- The built-in WeChat channel uses the latest inbound `context_token` as its reply key, so a valid `session_file` or `WECHAT_BOT_TOKEN` must exist before startup.
...
# WeChat (optional if session_file already contains bot_token)
...
1. Generate a session file with the built-in login CLI:
   ...
   If you are running directly from source without `uv`, you can use:
...
2. Enable `channels.wechat` in `config.yaml`, point `session_file` at that JSON file, and keep `cursor_file` on persistent storage so long-polling can resume without replaying old updates.
3. `WECHAT_BOT_TOKEN` is optional. If both are provided, `channels.wechat.bot_token` wins first, then `WECHAT_BOT_TOKEN`, then the token saved in `session_file`.
4. Start DeerFlow and chat with the connected WeChat account. DeerFlow reuses the sender's DeerFlow thread while replying with the latest inbound `context_token`.
