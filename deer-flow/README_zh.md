...
channels:
  ...
  wechat:
    ...
    # 可通过 `cd backend && uv run deerflow-wechat-login --session-file ../.state/wechat-link-session.json` 生成
    session_file: ./.state/wechat-link-session.json
    cursor_file: ./.state/wechat-cursor.json   # 建议持久化保存，避免重放旧消息
```
...
2. 在 `config.yaml` 中启用 `channels.wechat`，把 `session_file` 指向刚生成的 JSON 文件，并把 `cursor_file` 放到持久化目录，避免长轮询重放旧消息。
3. `WECHAT_BOT_TOKEN` 不是必填项；如果同时配置了 `channels.wechat.bot_token`、`WECHAT_BOT_TOKEN` 和 `session_file`，优先级依次为前者到后者。
4. 启动 DeerFlow 后，就可以直接在已登录的微信里与 DeerFlow 对话。通道内部会复用发送者的 DeerFlow thread，并使用最近一条入站消息的 `context_token` 进行回包。
