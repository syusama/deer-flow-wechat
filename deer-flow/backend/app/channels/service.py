...
_CHANNEL_CLASS_PATH_CONFIG_KEY = "class_path"
_NON_CHANNEL_CONFIG_KEYS = {"langgraph_url", "gateway_url", "session"}
...
class ChannelService:
    ...
    async def _start_channel(self, name: str, config: dict[str, Any]) -> bool:
        ...
        import_path = self._resolve_channel_import_path(name, config)
        if not import_path:
    ...
    @staticmethod
    def _resolve_channel_import_path(name: str, config: dict[str, Any]) -> str | None:
        configured_class_path = config.get(_CHANNEL_CLASS_PATH_CONFIG_KEY)
        if isinstance(configured_class_path, str) and configured_class_path.strip():
            return configured_class_path.strip()
        return _CHANNEL_REGISTRY.get(name)

    def _configured_channel_names(self) -> set[str]:
        names = set(_CHANNEL_REGISTRY)
        for name, config in self._config.items():
            if name in _NON_CHANNEL_CONFIG_KEYS or not isinstance(config, dict):
                continue
            names.add(name)
        return names
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        channels_status = {}
        for name in sorted(self._configured_channel_names()):
            config = self._config.get(name, {})
