...
uv run deerflow-wechat-login --session-file ../.state/wechat-link-session.json  # create/update WeChat session JSON
```
...
Bridges external messaging platforms (Feishu, Slack, Telegram, WeChat) to the DeerFlow agent via the LangGraph Server.
...
- `service.py` - Manages lifecycle of all configured channels from `config.yaml`; built-in channels come from `_CHANNEL_REGISTRY`, but `channels.<name>.class_path` can override or provide third-party channel implementations
- `slack.py` / `feishu.py` / `telegram.py` / `wechat.py` - Platform-specific implementations (`feishu.py` tracks the running card `message_id` in memory and patches the same card in place; `wechat.py` polls via `wechat-link` and maps inbound `context_token` to outbound replies)
- `wechat_session.py` - Shared helpers for reading/writing the WeChat login session JSON consumed by the channel
- `wechat_login.py` - CLI entry point behind `deerflow-wechat-login`; issues QR codes, waits for confirmation, and persists the session payload
...
- Per-channel configs: `feishu` (app_id, app_secret), `slack` (bot_token, app_token), `telegram` (bot_token), `wechat` (bot_token or session_file, cursor_file)
- WeChat auth precedence: `channels.wechat.bot_token` -> `WECHAT_BOT_TOKEN` -> token saved in `channels.wechat.session_file`
- WeChat bootstrap command: run `uv run deerflow-wechat-login --session-file ../.state/wechat-link-session.json` from `backend/` to generate the session JSON consumed by `wechat.py`
