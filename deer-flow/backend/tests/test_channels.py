...
class TestChannelService:
    ...
    def test_class_path_allows_custom_channel_name(self):
        from app.channels.service import ChannelService
        ...
        async def go():
            service = ChannelService(
                channels_config={
                    "custom": {
                        "enabled": True,
                        "class_path": "tests.test_channels:DummyChannel",
                    }
                }
            )
            try:
                await service.start()
                assert "custom" in service._channels
                assert service._channels["custom"].__class__.__name__ == "DummyChannel"
                assert service._channels["custom"].name == "dummy"
            finally:
                await service.stop()

        _run(go())

    def test_class_path_overrides_builtin_channel_registry(self):
        from app.channels.service import ChannelService
        ...
        async def go():
            ...
            try:
                ...
                assert service._channels["telegram"].__class__.__name__ == "DummyChannel"
                assert service._channels["telegram"].name == "dummy"
            finally:
                await service.stop()

        _run(go())

    def test_builtin_wechat_channel_can_start(self, monkeypatch, tmp_path):
        from app.channels.service import ChannelService
        from app.channels.wechat import WeChatChannel

        monkeypatch.setattr(WeChatChannel, "_create_client", lambda self, **kwargs: MagicMock(close=MagicMock()))
        monkeypatch.setattr(WeChatChannel, "_create_cursor_store", lambda self, path: MagicMock(load=MagicMock(return_value=None), save=MagicMock()))
        monkeypatch.setattr(WeChatChannel, "_create_polling_thread", lambda self: MagicMock(start=MagicMock(), join=MagicMock()))
            ...
            service = ChannelService(
                channels_config={
                    "wechat": {
                        ...
                        "cursor_file": str(tmp_path / "wechat-cursor.json"),
                    }
                }
            )
            try:
                await service.start()
                assert "wechat" in service._channels
                assert isinstance(service._channels["wechat"], WeChatChannel)
            finally:
                await service.stop()
